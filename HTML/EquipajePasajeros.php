      ...
      <div class="row">
        <div class="col-12">
          <nav class="main-nav">
            ...
            <!-- ***** Menu Start ***** -->
            <ul class="nav">
              <li><a><i class="fa-solid fa-calendar-days"></i>
                  <?php echo $Fecha_Ida; ?>
                </a></li>
              <li><a><i class="fa-solid fa-user-plus"></i>
                  <?php echo $NPasajeros; ?>
                </a></li>
              <li><a> <i class="fa-solid fa-plane-departure fa-beat-fade"></i>
                  <?php echo $Origen; ?>
                </a></li>
              <li><a><i class="fa-solid fa-bus fa-beat-fade"></i></a></li>
              <li><a><i class="fa-solid fa-plane-arrival fa-beat-fade"></i>
                  <?php echo $Destino; ?>
                </a></li>
              <li><progress class="progress" value="2" max="5"></progress></li>
              <li><a> <i class="fa-solid fa-cart-shopping"></i> $
                  <?php echo $formatted_double_price; ?>
                </a></li>
            </ul>
...
$url = 'Pago.php?ID_etiqueta=' . urlencode($ID_etiqueta) .
    '&Origen=' . urlencode($Origen) .
    '&Destino=' . urlencode($Destino) .
    '&Fecha_Ida=' . urlencode($Fecha_Ida) .
    '&Fecha_Regreso=' . urlencode($Fecha_Regreso) .
    '&NPasajeros=' . urlencode($NPasajeros);
?>

  <h2>Comprador</h2>
  <p>Deberá proporcionar toda la información del lote de pasajeros que viajaron con usted al presentarse en la compañía.
  </p>
  <div class="container1">
    <form id="form" action="<?php echo $url; ?>" method="post">
      ...
      <button class="BTN" type="submit">Siguinte > </button>
    </form>
  </div>
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        var fechaMinima = new Date(fechaHoy.getFullYear() - 18, fechaHoy.getMonth(), fechaHoy.getDate());

        inputFecha.addEventListener('input', function() {
            var fechaSeleccionada = new Date(inputFecha.value);

            if (fechaSeleccionada > fechaHoy || fechaSeleccionada >= fechaMinima) {
                alert('La fecha de nacimiento debe ser válida y corresponder a una persona mayor de 18 años.');
                inputFecha.value = ''; // Limpiar el campo si la fecha no es válida
            }
        });
    });
</script>
